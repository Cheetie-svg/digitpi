...
export default function VendorScanPage() {
  ...
  const processCode = async (raw: string) => {
    ...
    if (error) {
      ...
      setIsError(false);
      setMessage("Success! Order is now Dispatched.");
    }
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setIsError(true);
      setMessage(
        "Camera failed. Use phone browser or paste order ID below. " +
          (err?.message || "")
      );
    }
  };
      ...
      <div className="w-full max-w-sm bg-[#142A2E] rounded-[2.5rem] p-2.5 shadow-2xl">
        <div className="bg-[#EEF2F1] rounded-[2rem] overflow-hidden min-h-[620px] flex flex-col">
          ...
          <div className="flex items-center gap-3 px-5 pt-6 pb-3">
            ...
            <div>
              <h1 className="text-[16px] font-bold text-[#142A2E]">Scan Pickup QR</h1>
              <p className="text-[11px] text-[#8B9998]">Confirm handover to Delivery</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 space-y-4 pb-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm">
              ...
              <p className="text-[12px] text-[#6B7A79] leading-relaxed">
                Best on phone. Allow camera permission when asked.
              </p>
              <button
                onClick={startCamera}
                disabled={scanning || loading}
                className="mt-3 w-full bg-[#0F6E76] text-white rounded-full py-3 text-[14px] font-medium"
              >
                {scanning ? "Scanning..." : "Open Camera"}
              </button>
              <div id="qr-reader" className="mt-3 rounded-xl overflow-hidden" />
            </div>
